# gpu_backend.jl — AMD GPU utilities for RandomCloud AMDGPU extension
#
# Implements: a_gpu, de_gpu, estimar_vram, verificar_gpu, _ejecutar_gpu
# Uses ROCArray instead of CuArray, AMDGPU instead of CUDA.

# --- Data transfer utilities ---

"""
    RandomCloud.a_gpu(x::AbstractArray{Float64}) → ROCArray{Float32}

Convert Float64 array to Float32 and transfer to AMD GPU device memory.
"""
function RandomCloud.a_gpu(x::AbstractArray{Float64})
    return ROCArray(Float32.(x))
end

"""
    RandomCloud.de_gpu(x::ROCArray{Float32}) → Array{Float64}

Transfer ROCArray back to host memory and convert Float32 → Float64.
"""
function RandomCloud.de_gpu(x::ROCArray{Float32})
...
Check that AMDGPU/ROCm is functional and a device is available.
Throws an informative `ArgumentError` if not.
"""
function RandomCloud.verificar_gpu()
    if !AMDGPU.functional()
        throw(ArgumentError(
            "No AMD ROCm-capable GPU detected. Set gpu=false or install ROCm drivers."
        ))
    end
    return nothing
end
...
and a 20% overhead margin for the ROCm allocator.

Throws `ArgumentError` if the estimate exceeds available VRAM minus 500 MB safety margin.
"""
function RandomCloud.estimar_vram(config::ConfiguracionNube, entradas::Matrix{Float64})
    ...
    # 20% overhead for allocator fragmentation
    estimated = total * 1.2

    # Use device VRAM limit if available, otherwise default to 3.5 GB
    vram_limit = 3.5e9
    try
        dev = AMDGPU.device()
        vram_limit = Float64(AMDGPU.Runtime.device_memory_total(dev)) - 500e6  # 500 MB safety margin
    catch
        # If we can't query device, use conservative default
    end

    if estimated > vram_limit
        error_msg = "Estimated VRAM: $(round(estimated / 1e9, digits=2)) GB exceeds limit " *
                    "($(round(vram_limit / 1e9, digits=2)) GB). " *
...
function _elegir_estrategia(config::ConfiguracionNube, n_features::Int, n_samples::Int)
    ...
    total += topo[end] * n_samples * sizeof(Float32)
    for i in 1:(length(topo) - 1)
        total += topo[i + 1] * topo[i] * N * sizeof(Float32)
        total += topo[i + 1] * N * sizeof(Float32)
    end
    max_layer = maximum(topo[2:end])
    total += max_layer * n_samples * 2 * sizeof(Float32)
    estimated = total * 1.2
    ...
    try
        dev = AMDGPU.device()
        vram_limit = Float64(AMDGPU.Runtime.device_memory_total(dev)) - 500e6
    catch; end

    if estimated < vram_limit * 0.85
        return :gpu_batch
    else
        return :error
    end
...
Full AMD GPU execution path for the Random Cloud Method.
Exploration runs on CPU (topology changes per network), refinement runs on GPU.
"""
function RandomCloud._ejecutar_gpu(motor::MotorNube)
    ...
    if estrategia === :error
        throw(ArgumentError("Problem too large for GPU. Reduce tamano_nube or dataset size."))
    end
    ...
    if mejor_red !== nothing
        ...
        pesos_gpu = [ROCArray(Float32.(mejor_red.pesos[l])) for l in eachindex(mejor_red.pesos)]
        biases_gpu = [ROCArray(Float32.(mejor_red.biases[l])) for l in eachindex(mejor_red.biases)]
        ...
        try
            dev = AMDGPU.device()
            used = AMDGPU.Runtime.device_memory_total(dev) - AMDGPU.Runtime.device_memory_free(dev)
            pico_vram = max(pico_vram, Float64(used))
        catch; end
        ...
        # Track peak VRAM after training
        try
            dev = AMDGPU.device()
            used = AMDGPU.Runtime.device_memory_total(dev) - AMDGPU.Runtime.device_memory_free(dev)
            pico_vram = max(pico_vram, Float64(used))
