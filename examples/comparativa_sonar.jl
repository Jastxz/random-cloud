# =============================================================================
# Comparativa: Nube Aleatoria vs Clásico en Sonar
# =============================================================================
#
# Ejecutar con:
#   julia --project=. -t auto examples/comparativa_sonar.jl
#
# Sonar: 208 muestras, 60 features, 2 clases (minas vs rocas).
# Ratio features/muestras alto (60/208). Clásico en papers de NAS.
# =============================================================================
...
features = zeros(Float64, 60, n_total)
...
for (k, line) in enumerate(lines)
    ...
    labels[k] = parts[61] == "M" ? 1 : 0  # Mine=1, Rock=0
end
...
pct_mine = round(sum(labels) / n_total * 100, digits=1)
println("  Muestras: $n_total | Features: 60 | Clases: 2 (Mine/Rock)")
println("  Distribución: $(pct_mine)% minas, $(round(100-pct_mine, digits=1))% rocas")
println()
...
println("=" ^ 75)
println("  COMPARATIVA: Nube Aleatoria vs Clásico — Sonar")
println("=" ^ 75)
println()
println("  Épocas: $EPOCAS | LR: $LR | Threads: $(Threads.nthreads())")
println("  Nube: 50 redes, umbral=0.55, eliminar=1")
println()
...
for topo in topologias
    ...
    for intento in 1:3
        ...
        config = ConfiguracionNube(
            tamano_nube=50, topologia_inicial=topo, umbral_acierto=0.55,
