# =============================================================================
# Comparativa: Método de la Nube Aleatoria vs Entrenamiento Clásico en Wine
# =============================================================================
#
# Ejecutar con:
#   julia --project=. examples/comparativa_wine.jl
#
...
labels = vec(dataset.targets)            # Vector{Int} (1, 2, 3)

n_muestras = size(features, 2)

# Normalizar features a [0,1] por fila (min-max)
...
objetivos = zeros(Float64, n_clases, n_muestras)
for k in 1:n_muestras
    objetivos[labels[k], k] = 1.0
end

println("  Features: $(size(features)) — 13 atributos × $n_muestras muestras")
println("  Clases:   $n_clases")
println()
...
println("=" ^ 80)
println("  COMPARATIVA: Nube Aleatoria vs Clásico — Wine")
println("=" ^ 80)
println()
